...
# Initialize the LLM for use with router / structured output - Using Google Gemini
...
def update_memory(store, namespace, messages):
    ...
    user_preferences = store.get(namespace, "user_preferences")
    # Update the memory - Using Google Gemini
    llm = init_chat_model("gemini-2.5-flash", model_provider="google_genai", temperature=0.0).with_structured_output(UserPreferences)
    result = llm.invoke(
        [
